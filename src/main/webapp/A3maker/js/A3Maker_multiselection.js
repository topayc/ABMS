...
$(function () {
    A3Maker.multiSelectionRect= {
        ...
        initializeSelectionRectEventHandler: function (selector) {
            ...
            var context = this;
            $(A3Maker.multiSelectStr).appendTo(targetContainer);
            this.$multiSelectRect = $('.multi_select').width(0).height(0).hide();
            
            /*컨테이너 영역에서도  선택 기능이 가능하도록 하려면  selector 를 A3Maker.holder 로 변경 
             A3Maker.page
             #vertical_table
             * */
            $(document).on('mousedown', '#vertical_table', /*A3Maker.holder*/  function (event) {
                if (event.ctrlKey) {
            ...
            .bind('mouseup', function (event) {
                if (context.curSelectionAction == context.selectionActionEnum.drag) {
                    ...
                    if (context.selectionComponentArr.length > 0) {
                        if (A3Maker.getSelectedComponent()) A3Maker.getSelectedComponent().setSelectStatus(false);
       ...
       removeComponentToSelection: function (component) {
            ...
            $.each(this.selectionComponentArr, function (inx, com) {
                if (com.data.UUID == component.data.UUID) {
                    index = inx;
                    return;
                }
            });
		...
		*/
        ...
        getComponentsInSelectionRect: function (x, y, right, bottom) {
            ...
            var context = this;
            var searchContainer = A3Maker.getSelectedPage();
            ...
            else if (selectedComponent.data.type == "listrow" || 
            		selectedComponent.data.type == "popup" || 
            		selectedComponent.data.type == "list") {
        ...
        initializeAlignDialogEventHandler: function () {
            $(A3Maker.selectionAfterSelectionStr).appendTo('body');
        ...
        alignLeftMost : function (){
            ...
            $.each(this.selectionComponentArr, function (index, com) {
                if (index == 0) {
                    ...
                    com.setPositionAndWidth(sCom.data.left, com.data.top, com.data.width, com.data.height);
                }
            });
        },
        ...
        alignTopMost: function () {
            ...
            $.each(this.selectionComponentArr, function (index, com) {
                if (index == 0) {
                    ...
                    com.setPositionAndWidth(com.data.left, sCom.data.top, com.data.width, com.data.height);
                }
            });
        },
        ...
        alignBottomMost: function () {
            this.selectionComponentArr.sort(function (a, b) {
                return (b.data.top + b.data.height) - (a.data.top + a.data.height);
            });
            ...
            $.each(this.selectionComponentArr, function (index, com) {
                if (index == 0) {
                    ...
                    com.setPositionAndWidth(com.date.left, (sCom.data.top + sCom.data.height) - com.data.height, com.data.width, com.data.height);
                }
            });
        },
        ...
        alignRightMost: function () {
            this.selectionComponentArr.sort(function (a, b) {
                return (b.data.left + b.data.width) - (a.data.left + a.data.width);
            });
            ...
            $.each(this.selectionComponentArr, function (index, com) {
                if (index == 0) {
                    ...
                    com.setPositionAndWidth((sCom.data.left + sCom.data.width) - com.data.width,  com.data.top, com.data.width, com.data.height);
                }
            });
        },
        ...
        alignHorizontalOffset: function () {
            ...
            var startX = firstCom.data.left + (firstCom.data.width/ 2);
            var endX = lastCom.data.left + (lastCom.data.width / 2);
            var offset = parseInt((endX - startX) / (this.selectionComponentArr.length - 1));
            $.each(this.selectionComponentArr, function (inx, com) {
                com.setPositionAndWidth(
                    (startX + (offset * inx)) - (com.data.width/ 2),
                    com.data.top,
                    com.data.width,
                    com.data.height);
            });
        ...
        alignVerticalOffset: function () {
            ...
            var firstCom = this.selectionComponentArr[0];
            var lastCom = this.selectionComponentArr[this.selectionComponentArr.length - 1];
            var startY = firstCom.data.top + (firstCom.data.height / 2)  ;
            var endY = lastCom.data.top + (lastCom.data.height / 2);
            var offset = parseInt((endY - startY) / (this.selectionComponentArr.length -1));
            $.each(this.selectionComponentArr, function (inx, com) {
                com.setPositionAndWidth(
                    com.data.left,  
                    (startY + (offset * inx))- (com.data.height/2),
                    com.data.width,
                    com.data.height);
            });
        },
		...
		fillContainerWithComponent : function(){
			...
			$.each(this.selectionComponentArr, function (inx, com) {
				var parentContainer = com.parent;
				 ...
				 var width = parentContainer.data.width;
				 var height = parentContainer.data.height;
				 //부모 컨테이너이 크기를 꽉 차게 할 때는 .고정 종횡비 기능 off
				 com.data.fixRatio = false;
